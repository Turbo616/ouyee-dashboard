...
const TAB_RULES = [
  { siteId: "ouyedisplay", domain: "ouyedisplay.com", mode: "exact", key: "\u6b27\u91ce1" },
  { siteId: "ouyedisplay", domain: "ouyedisplay.com", mode: "contains", key: "whatsapp" },
...
// Fixed columns from your workbook screenshot:
...
function matchRuleForTab(title) {
  ...
  return TAB_RULES.find((r) => {
    const key = normalize(r.key);
    if (r.mode === "exact") return t === key;
...
export const onRequestGet = async (context) => {
  ...
  try {
    ...
    const ranges = selected.map((t) => `${t}!A:J`);
    ...
    for (const vr of batch.valueRanges || []) {
      ...
      for (let i = 1; i < rows.length && i <= maxRows; i += 1) {
        ...
        const sourceUrl = row[IDX_SOURCE_URL] || "";
        const inferred = inferSiteByUrl(sourceUrl);
        ...
        const country = row[IDX_COUNTRY] || "";
        const storeType = row[IDX_STORE] || "";
        const owner = row[IDX_OWNER] || "";
